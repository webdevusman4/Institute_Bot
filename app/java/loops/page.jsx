...
import javaData from "../../../data/JavaLoops.json";
...
export default function JavaPreview() {
    ...
    const renderText = (text) => {
        ...
        return parts.map((part, i) => {
            if (part.startsWith("**") && part.endsWith("**")) {
                return (
                    <strong key={i} className="font-extrabold text-slate-900 bg-purple-50/50 px-1 rounded">
                        {part.slice(2, -2)}
                    </strong>
                );
            }
                ...
                {parts.map((part, index) => {
                    ...
                    return (
                        <div key={index} className="my-10 flex flex-col items-center bg-slate-50 p-3 md:p-4 rounded-2xl border border-slate-200 shadow-inner">
                            ...
                            />
                            <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-purple-400 mt-4 mb-1 text-center font-mono">
                                // Figure: {part}
                            </span>
                        </div>
                    );
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-purple-200 selection:text-purple-900">
            ...
            <aside className="w-80 bg-white border-r border-slate-200 p-6 hidden md:block sticky top-0 h-screen overflow-y-auto shadow-sm z-40">
                <div className="mb-10 mt-4">
                    <span className="bg-purple-50 text-purple-700 px-3 py-1.5 rounded-md text-xs font-black uppercase tracking-widest mb-4 inline-block border border-purple-100">
                        BSSE-351
                    </span>
                    ...
                    {javaData.sections.map((section) => (
                        <button
                            ...
                            className={`w-full text-left px-5 py-4 rounded-xl font-bold transition-all duration-200 border group ${activeSection === section.id
                                    ? "bg-purple-600 text-white border-purple-600 shadow-lg shadow-purple-200/50 scale-[1.02]"
                                    : "bg-slate-50 text-slate-600 border-slate-200 hover:border-purple-200 hover:bg-white hover:shadow-md"
                                }`}
                        >
                            <span className={`text-[10px] uppercase tracking-widest block mb-1.5 font-black ${activeSection === section.id ? "text-purple-200" : "text-slate-400 group-hover:text-purple-400"
                                }`}>
                                Module {section.id}
                            </span>
            ...
            <main className="flex-1 p-5 md:p-12 lg:p-16 max-w-4xl mx-auto w-full">
                ...
                <header className="mb-10 md:mb-14">
                    <div className="inline-flex items-center gap-3 mb-3">
                        ...
                        <h2 className="text-purple-600 font-black uppercase tracking-widest text-xs md:text-sm">
                        ...
                        return (
                            <div
                                ...
                                className={`bg-white border rounded-2xl transition-all duration-300 overflow-hidden ${isOpen
                                        ? "border-purple-300 shadow-xl shadow-purple-100/50 ring-1 ring-purple-50"
                                        : "border-slate-200 hover:border-purple-200 shadow-sm hover:shadow-md"
                                    }`}
                            >
                                {/* Accordion Toggle Button */}
                                <button
                                    onClick={() => setOpenSubtopic(isOpen ? null : sub.id)}
                                    className={`w-full flex justify-between items-center p-5 md:p-7 text-left transition-colors group ${isOpen ? "bg-purple-50/30" : "hover:bg-slate-50"
                                        }`}
                                >
                                    <div className="pr-4">
                                        <span className="text-[10px] md:text-xs font-black text-purple-500 uppercase tracking-widest block mb-1.5 font-mono">
                                            // Topic {sub.id}
                                        </span>
                                    ...
                                    <div
                                        className={`shrink-0 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full transition-transform duration-300 shadow-sm border ${isOpen
                                                ? "bg-purple-600 text-white border-purple-600 rotate-180"
                                                : "bg-white text-slate-400 border-slate-200 group-hover:border-purple-300 group-hover:text-purple-500"
                                            }`}
                                    >
                                    ...
                                    <div className="p-5 md:p-8 pt-0 border-t border-slate-100 bg-white">
                                        ...
                                        {sub.list_items && (
                                            <div className="bg-purple-50/50 p-6 md:p-8 rounded-2xl border border-purple-100">
                                                <h4 className="font-black text-purple-900 uppercase text-xs md:text-sm tracking-widest mb-6 flex items-center gap-2">
                                                    <svg className="w-4 h-4 text-purple-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"></path><path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd"></path></svg>
                                                    {sub.list_title}
                                                </h4>
                                                <ul className="space-y-4">
                                                    {sub.list_items.map((item, i) => (
                                                        <li key={i} className="flex gap-4 text-slate-700 leading-relaxed text-sm md:text-base">
                                                            <span className="text-purple-500 font-black mt-1 text-lg leading-none shrink-0 font-mono">
                                                                {'>'}
                                                            </span>
                                        ...
                                        <div className="mt-8 pt-6 border-t border-slate-100 flex justify-end">
                                            <button
                                                ...
                                                className="bg-slate-50 hover:bg-purple-50 text-slate-600 hover:text-purple-700 font-bold py-2.5 px-6 rounded-xl transition-colors text-sm flex items-center gap-2 border border-slate-200 hover:border-purple-200"
                                            >
                                                Mark as Complete
