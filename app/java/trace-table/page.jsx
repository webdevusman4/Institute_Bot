...
import javaData from "../../../data/JavaTraceTable.json";
...
export default function JavaPreview() {
    ...
    return (
        <div className="flex flex-col md:flex-row min-h-screen bg-slate-50 font-sans text-slate-900 selection:bg-amber-200">
            ...
            <aside className="w-80 bg-white border-r border-slate-200 p-6 hidden md:block sticky top-0 h-screen overflow-y-auto shadow-sm z-40">
                <div className="mb-10 mt-4">
                    <span className="bg-amber-100 text-amber-700 px-3 py-1.5 rounded-md text-xs font-black uppercase tracking-widest mb-4 inline-block shadow-sm">
                        BSSE-351
                    </span>
                    <h1 className="text-3xl font-black text-slate-900 leading-tight tracking-tight">
                        Java <br />
                        <span className="text-amber-600">Fundamentals</span>
                    </h1>
                </div>
                    ...
                    {javaData.sections.map((section) => (
                        <button
                            ...
                            className={`w-full text-left px-5 py-4 rounded-xl font-bold transition-all duration-200 border group ${activeSection === section.id
                                ? "bg-amber-600 text-white border-amber-600 shadow-lg shadow-amber-200/50 scale-[1.02]"
                                : "bg-slate-50 text-slate-600 border-slate-200 hover:border-amber-300 hover:bg-white hover:shadow-md"
                                }`}
                        >
                            <span className={`text-[10px] uppercase tracking-widest block mb-1.5 font-black ${activeSection === section.id ? "text-amber-200" : "text-slate-400 group-hover:text-amber-400"
                                }`}>
                                Section {section.id}
                            </span>
            ...
            <main className="flex-1 p-5 md:p-12 lg:p-16 max-w-4xl mx-auto w-full">
                ...
                <header className="mb-10 md:mb-14">
                    <div className="inline-flex items-center gap-3 mb-3">
                        ...
                        <h2 className="text-amber-600 font-black uppercase tracking-widest text-xs md:text-sm">
                        ...
                        return (
                            <div
                                ...
                                className={`bg-white border rounded-2xl transition-all duration-300 overflow-hidden ${isOpen
                                    ? "border-amber-300 shadow-xl shadow-amber-100/50 ring-1 ring-amber-100"
                                    : "border-slate-200 hover:border-amber-200 shadow-sm hover:shadow-md"
                                    }`}
                            >
                                {/* Accordion Toggle Button */}
                                <button
                                    onClick={() => setOpenSubtopic(isOpen ? null : sub.id)}
                                    className={`w-full flex justify-between items-center p-5 md:p-7 text-left transition-colors group ${isOpen ? "bg-amber-50/40" : "hover:bg-slate-50"
                                        }`}
                                >
                                    <div className="pr-4">
                                        <span className="text-[10px] md:text-xs font-black text-amber-500 uppercase tracking-widest block mb-1.5">
                                            Topic {sub.id}
                                        </span>
                                        <h3 className="text-lg md:text-xl font-bold text-slate-800 group-hover:text-amber-700 transition-colors">
                                            {sub.title}
                                        </h3>
                                    </div>
                                    <div
                                        className={`shrink-0 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full transition-transform duration-300 shadow-sm border ${isOpen
                                            ? "bg-amber-600 text-white border-amber-600 rotate-180"
                                            : "bg-white text-slate-400 border-slate-200 group-hover:border-amber-300 group-hover:text-amber-500"
                                            }`}
                                    >
                                ...
                                {isOpen && (
                                    <div className="p-5 md:p-8 pt-0 border-t border-slate-100 bg-white">
                                        ...
                                        {sub.formula && (
                                            <div className="bg-slate-900 text-amber-300 p-6 md:p-8 rounded-2xl font-mono text-center text-base md:text-lg shadow-inner mb-8 border border-slate-800 relative overflow-hidden">
                                        ...
                                        {/* Content Lists */}
                                        {sub.list_items && (
                                            <div className="bg-amber-50/50 p-6 md:p-8 rounded-2xl border border-amber-100">
                                                <h4 className="font-black text-amber-900 uppercase text-xs md:text-sm tracking-widest mb-6 flex items-center gap-2">
                                                    <svg className="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"></path><path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd"></path></svg>
                                                    {sub.list_title}
                                                </h4>
                                                <ul className="space-y-4">
                                                    {sub.list_items.map((item, i) => (
                                                        <li key={i} className="flex gap-4 text-slate-700 leading-relaxed text-sm md:text-base">
                                                            <span className="text-amber-500 font-black mt-1 text-lg leading-none shrink-0">
                                                                ↳
                                                            </span>
                                        ...
                                        <div className="mt-8 pt-6 border-t border-slate-100 flex justify-end">
                                            <button
                                                ...
                                                className="bg-amber-50 hover:bg-amber-100 text-amber-700 font-bold py-2.5 px-6 rounded-xl transition-colors text-sm flex items-center gap-2 border border-amber-200 hover:border-amber-300"
                                            >
                                                Complete & Close
